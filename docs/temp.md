...
本 DSL 旨在**以纯文本形式描述节点图**。它利用 TypeScript 的语法高亮特性，使得复杂的逻辑图变得易于阅读、编写、版本控制和审查。
...
-   `false` 分支：循环**正常**结束 (`i > end`) 后执行一次的内容。
...
### 5.2. `function output` (获取输出变量)

节点可以输出变量供后续节点使用。使用 `[my_var_name]` 语法捕获这些输出。
...
系统支持的所有数据类型如下表所示, 在运行中均是以类的形式存在的.

| 序号 | 类型 | 类型名 | 备注 |
| :---: | :--- | :--- | :--- |
| 1 | 整型 | Int | 封装 bigint 的类，同时提供 `int` 类型作为 `bigint` 的别名以简化代码 |
| 2 | 浮点型 | Float | 封装 number 的类，同时提供 `float` 类型作为 `number` 的别名以简化代码 |
| 3 | 布尔型 | Bool | 封装 boolean 的类，同时提供 `bool` 类型作为 `boolean` 的别名以简化代码 |
| 4 | 字符串 | Str | 封装 string 的类，同时提供 `str` 类型作为 `string` 的别名以简化代码 |
| 5 | 三维向量 | Vec | 封装 x, y, z 三个 float 值的类 |
| 6 | GUID | GUID | 内部存储值为 int，但不可转为 int |
| 7 | 实体 | Entity | 内部存储值为 int，但不可转为 int，唯一不允许设置初始值的对象类型 |
