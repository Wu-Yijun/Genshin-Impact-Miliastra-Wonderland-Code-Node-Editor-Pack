 ...
 */
`;
  const FILE_DATA_CONST = "export const DATA = " + inspect(doc, { maxArrayLength: Infinity, depth: Infinity }) + " as const satisfies Document;";
  const FILE_DATA_DEFAULT = "export default DATA;";
  const FILE_DATA = "export const data: Document = DATA;";

  const FILE_DECLARATION = doc.Schema;

  writeFileSync(DIR + "data.ts", [FILE_DESC, FILE_DATA_CONST, FILE_DATA, FILE_DATA_DEFAULT, FILE_DECLARATION].join("\n\n"));
}
